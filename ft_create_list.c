...
/*   Updated: 2017/01/31 16:37:13 by pguillie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static t_strout	*ft_lstnew(char *s, size_t len, t_fields f)
{
	t_strout	*new;
	size_t		i;
...
static int		ft_fields(const char *restrict s, t_fields *f)
{
	int			l;
	int			notype;
	int			n;
...
static int		ft_get(const char *restrict s, int *p, t_fields *f)
{
	int		l;
...
t_strout		*ft_create_list(const char *restrict str, t_strout *start)
{
	...
	while (str[i])
	{
		...
		if (!start)
		{
			start = ft_lstnew((char*)str + i, len, f);
			list = start;
		}
		else
		{
			list->next = ft_lstnew((char*)str + i, len, f);
			list = list->next;
		}
		i += len;
	}
